...
public class MakeMoveLogicDraughts implements MakeMoveLogic {
	...
	public void checkForValidMove() throws GameException {
		...
		} else if (newPositionX > board.getSize() - 1 || newPositionX < 0 || newPositionY > board.getSize() - 1
				|| newPositionY < 0) {
		...
		Counter middlePiece = (Counter) board
				.getPiece(new Position((currentX + newPositionX) / AVG, (currentY + newPositionY) / AVG));
		...
		case KING:
			if (Math.abs(newPositionX - currentX) == SHORT_MOVE && Math.abs(newPositionY - currentY) == SHORT_MOVE) { // valid
				// Move is going forward/backward by one so is valid, carry on.
					...
					&& (Math.abs(newPositionY - currentY) == LONG_MOVE) && !middlePiece.equals(null)
